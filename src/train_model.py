...
from dataset import build_zjut
...
# 如果目录不存在，创建目录
if not os.path.exists("../model/WebModel/temp"):
    os.mkdir("../model/WebModel/temp")
...
# 准备数据集
train_dataset = build_zjut.load("../dataset", True)
test_dataset = build_zjut.load("../dataset", False)
...
# tensorboard, 指定日志名称
writer = SummaryWriter("../logs/demo_model")
...
for i in range(epoch):
    ...
    # 保存模型
    if i % 10 == 0:
        torch.save(model.state_dict(), "../model/WebModel/temp/demo__{}.pth".format(i))
    print("\n")
